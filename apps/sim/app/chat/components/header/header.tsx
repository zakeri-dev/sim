...
export function ChatHeader({ chatConfig, starCount }: ChatHeaderProps) {
  ...
  const primaryColor =
    chatConfig?.customizations?.primaryColor || "var(--brand-primary-hex)";
  const customImage =
    chatConfig?.customizations?.imageUrl || chatConfig?.customizations?.logoUrl;
    ...
    >
      <div className="flex items-center gap-[34px]">
        <div className="flex items-center gap-3">
          ...
          <h2
            className={`${inter.className} font-medium text-[18px] text-foreground`}
          >
